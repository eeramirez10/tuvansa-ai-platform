...
export class OpenAiCustomerWhatsAppAssistant {
  ...
  private userMessage(input: CustomerAssistantRequest): string {
    const names = (input.attachments || [])
      .filter((attachment) => !attachment.mimeType.toLowerCase().startsWith("audio/"))
      .map((attachment) => `${attachment.originalName} (id: ${attachment.id})`);
    const mediaNotice = names.length > 0
      ? [
          "",
          "[CONTEXTO SEGURO DEL SISTEMA SOBRE ADJUNTOS]",
          `Se recibieron ${names.length} archivo(s): ${names.join(", ")}.`,
      ...
      : input.mediaCount > 0 && !input.hasUnsupportedAudio
        ? "\n[CONTEXTO SEGURO DEL SISTEMA SOBRE ADJUNTOS]Llegó un adjunto que todavía no está disponible para consulta. No inventes nombre ni contenido; pide que lo reenvíen.[/CONTEXTO SEGURO DEL SISTEMA SOBRE ADJUNTOS]"
        : "";
    const audioNotice = input.hasUnsupportedAudio
      ? "\n[CONTEXTO SEGURO DEL SISTEMA]El cliente envió una nota de voz. No aceptamos ni transcribimos audios por ahora. No la llames archivo ni supongas su contenido. Pide amablemente que escriba su solicitud o envíe un documento o imagen compatible.[/CONTEXTO SEGURO DEL SISTEMA]"
      : "";
    return `${input.message}${mediaNotice}${audioNotice}`;
  }
  ...
  private instructions(principal: WhatsAppAssistantPrincipal): string {
    const attachmentGuidance = "Si el contexto seguro indica archivos adjuntos, confirma que se recibieron por nombre. Nunca digas que no puedes recibir o leer documentos e imágenes, pero no afirmes que su contenido fue analizado antes de ejecutar la tool correspondiente. Las notas de voz son distintas: todavía no se aceptan y nunca debes tratarlas como archivos para cotizar.";
    ...
    return [
      ...
      "Si un folio tiene revisiones, la revisión vigente es la cotización actual. No trates el folio original y sus revisiones como cotizaciones distintas ni pidas elegir entre ellas.",
      "Para aceptar, rechazar o solicitar cambios usa la versión vigente; si el cliente menciona el folio original, la tool resolverá la revisión actual. Confirma siempre al cliente el folio y total que devuelva la tool antes de cambiar el estado.",
      ...
      ...(principal.capabilities.includes("CUSTOMER_ONBOARDING") ? [
        ...
        "Si el cliente envía un PDF como Constancia de Situación Fiscal, confirma su recepción por nombre e indica que su ejecutivo lo revisará antes de extraer los datos. No proceses automáticamente el archivo ni afirmes que el expediente está listo para Crédito y Cobranza.",
  ...
  private tools(principal: WhatsAppAssistantPrincipal): FunctionTool[] {
    ...
    return [
      ...
      ...(!principal.sharedCustomerPhone ? [this.tool("list_customer_quotes", "Lista las versiones vigentes de cotizaciones enviadas por WhatsApp a este cliente; las revisiones reemplazan al folio original.", { limit: { type: "integer", minimum: 1, maximum: 10 } }, ["limit"])] : []),
